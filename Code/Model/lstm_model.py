...
def get_convolutional_lstm(input_shape, comb_degree):
    model = Sequential()
    hidden_cells = 64 * comb_degree
    model.add(ConvLSTM2D(hidden_cells, kernel_size=32, input_shape=input_shape, return_sequences=True, recurrent_dropout=0.2))
    model.add(ConvLSTM2D(hidden_cells, kernel_size=32 * 2))
    model.add(Flatten())
